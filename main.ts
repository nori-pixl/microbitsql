...
namespace sql {
     ...
     * For SELECT statements, use the result* blocks afterwards to read
     * the rows that were found.
     * Returns the number of rows affected/found, or -1 if the
     * statement could not be understood.
     */
    //% blockId=sql_query
    //% block="run SQL query %queryText"
    //% weight=60
    export function query(queryText: string): number {
        let q = queryText.trim()
        let upper = q.toUpperCase()

        if (upper.indexOf("CREATE TABLE") == 0) {
            let rest = q.substr("CREATE TABLE".length).trim()
            let parenStart = rest.indexOf("(")
            let parenEnd = rest.indexOf(")")
            if (parenStart < 0 || parenEnd < 0) return -1
            let tname = rest.substr(0, parenStart).trim()
            let colsStr = rest.substr(parenStart + 1, parenEnd - parenStart - 1)
            createTable(tname, colsStr)
            return 1
        }

        if (upper.indexOf("DROP TABLE") == 0) {
            let tname = q.substr("DROP TABLE".length).trim()
            dropTable(tname)
            return 1
        }

        if (upper.indexOf("INSERT INTO") == 0) {
            let rest = q.substr("INSERT INTO".length).trim()
            let valuesIdx = rest.toUpperCase().indexOf("VALUES")
            if (valuesIdx < 0) return -1
            let tname = rest.substr(0, valuesIdx).trim()
            let after = rest.substr(valuesIdx + "VALUES".length).trim()
            let parenStart = after.indexOf("(")
            let parenEnd = after.indexOf(")")
            if (parenStart < 0 || parenEnd < 0) return -1
            let valsStr = after.substr(parenStart + 1, parenEnd - parenStart - 1)
            insert(tname, valsStr)
            return 1
        }
        ...
        if (upper.indexOf("SELECT") == 0) {
            ...
            let whereIdx = upper.indexOf("WHERE")
            let tname: string
            let whereClause = ""
            if (whereIdx >= 0) {
                tname = q.substr(fromIdx + 4, whereIdx - (fromIdx + 4)).trim()
                whereClause = q.substr(whereIdx + 5).trim()
            } else {
                tname = q.substr(fromIdx + 4).trim()
            }
            if (whereClause.length > 0) {
                let cond = parseCondition(whereClause)
                return select(tname, cond.column, cond.op, cond.value)
            } else {
                return selectAll(tname)
            }
        }

        if (upper.indexOf("UPDATE") == 0) {
            let rest = q.substr("UPDATE".length).trim()
            let setIdx = rest.toUpperCase().indexOf("SET")
            if (setIdx < 0) return -1
            let whereIdx = rest.toUpperCase().indexOf("WHERE")
            let tname = rest.substr(0, setIdx).trim()
            let setClause: string
            let whereClause = ""
            if (whereIdx >= 0) {
                setClause = rest.substr(setIdx + 3, whereIdx - (setIdx + 3)).trim()
                whereClause = rest.substr(whereIdx + 5).trim()
            } else {
                setClause = rest.substr(setIdx + 3).trim()
            }
            let eqIdx = setClause.indexOf("=")
            if (eqIdx < 0) return -1
            let setCol = setClause.substr(0, eqIdx).trim()
            let setVal = stripQuotes(setClause.substr(eqIdx + 1))
            if (whereClause.length > 0) {
                let cond = parseCondition(whereClause)
                return update(tname, setCol, setVal, cond.column, cond.op, cond.value)
            } else {
                return update(tname, setCol, setVal, "", CompareOp.Equal, "")
            }
        }

        if (upper.indexOf("DELETE FROM") == 0) {
            let rest = q.substr("DELETE FROM".length).trim()
            let whereIdx = rest.toUpperCase().indexOf("WHERE")
            let tname: string
            let whereClause = ""
            if (whereIdx >= 0) {
                tname = rest.substr(0, whereIdx).trim()
                whereClause = rest.substr(whereIdx + 5).trim()
            } else {
                tname = rest.trim()
            }
            if (whereClause.length > 0) {
                let cond = parseCondition(whereClause)
                return deleteRows(tname, cond.column, cond.op, cond.value)
            } else {
                return deleteRows(tname, "", CompareOp.Equal, "")
            }
        }
